# Docs and experiment results can be found at https://docs.cleanrl.dev/rl-algorithms/ppo/
# Also see
#  - 11 core impl details on PPO: https://www.youtube.com/watch?v=MEt6rrxH8W4, by Costa Huang
#  - 37 impl details on PPO: https://iclr-blog-track.github.io/2022/03/25/ppo-implementation-details/, by Costa Huang
#  - ppo-implementation-details: https://github.com/vwxyzjn/ppo-implementation-details/tree/main, by Costa Huang
...
def parse_args():
    ...
    parser.add_argument("--repo-id", type=str, default="user05181824/ppo-CartPole-v1", help="id of the model repository from the Hugging Face Hub {username/repo_name}")
...
if __name__ == "__main__":
    ...
    writer_dir = os.path.dirname(__file__) + f"/runs/{run_name}"
    print(f"Writing to {writer_dir}")
    ...
    # Note that we use the same optimizer for policy update and value update.
    ...
    for update in range(1, num_updates + 1):
        ...
        for step in range(0, args.num_steps):
            ...
            if "_episode" in info and info["_episode"].any():
                # For simplicity, we only collect stats from the first complete episode.
                idx = np.argmax(info["_episode"])
                print(f"global_step={global_step}, episodic_return={info['episode']['r'][idx]:.2f}")
        ...
        with torch.no_grad():
            next_value = agent.get_value(next_obs).reshape(1, -1)  # (num_envs, obs_dim)
            if args.gae:
